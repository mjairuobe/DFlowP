...
- **Eigenes Repo** (Template ist das Repo-Root): `git rev-parse HEAD:<pfad>` und `docker build .` wie gewohnt.
- **Monorepo** (Template liegt unter `modular-ci-template/`): Tree-Hashes nutzen Pfade relativ zum Git-Root; `ci_docker_build.py` baut mit Kontext auf dem Git-Root und `-f modular-ci-template/Dockerfile`. Das Jenkinsfile wechselt automatisch in `modular-ci-template/`, falls der Ordner existiert.
